...
from cuckoo_time_translator.cpp import LocalTime, RemoteTime
...
class TimestampFilter:
  ...
  def __init__(self, owt, batch=False, switchTime=None):
    if switchTime:
      self.owt = cpp.SwitchingOwt(switchTime, owt)
    else:
      self.owt = owt
    self.batch = batch
...
class ConvexHullFilter (TimestampFilter):
  ...
  def __init__(self, *args, **kwargs):
    TimestampFilter.__init__(self, cpp.ConvexHullOwt(), *args, **kwargs)
